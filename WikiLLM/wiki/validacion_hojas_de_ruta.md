...
| Alto de puerta (`A` sin reveal) en `B`/`UB`/`V`, mismo patrón de `frente` que BFD/SBFD/SVFD/UBFD/VFD/W/WBL pero sin el `-RV` | Corregido a `A-RV`, confirmado por el usuario como regla general de muebles de puertas ("altura del mueble - 3.2mm"). Verificado con `calcularMueble()`: 758,80mm exacto en los 5 tipos para A=30" | `0036` |
