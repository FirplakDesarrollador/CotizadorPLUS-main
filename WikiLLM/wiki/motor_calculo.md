...
- **Tarugos estructurales:** Cada `base`, `tapa`, `base_tapa` o `base_tapa_division` consume 8 tarugos por unidad (4 por lateral); `base_gaveta` consume 0. La regla se persiste en la plantilla y el motor multiplica `cantidad × tarugos` antes de aplicar el precio del consumible `tarugo`.
- **Barras estabilizadoras:** Cada pieza `trasero_gaveta*` con ancho de 183mm agrega un par `BARRAEST`. El motor lo deriva del despiece evaluado y reemplaza la fórmula heredada de la plantilla, por lo que el conteo sigue los traseros grandes incluso en tipologías nuevas.
- **DB-4:** Sus cuatro `trasero_gaveta` se cortan a 68mm. Por no tener traseros de 183mm, no agrega barras estabilizadoras bajo la regla anterior.
- **Cantidad de consumibles en el desglose:** Además del costo, el motor expone `cantidadesConsumibles` (tarugos, soportes, cartón y etiquetas). El Simulador usa esa cantidad en la tabla Materiales, de modo que Tarugos muestra unidades reales y el valor continúa siendo `cantidad × precio unitario`.
- **Soporte de entrepaño:** El selector `soporte` usa el consumible activo `SOPORTE ENTREPAÑO METALICO 5MM`, a $45,7 por unidad. La carga del cálculo filtra herrajes inactivos para que un reemplazo de catálogo no deje precios ambiguos.
